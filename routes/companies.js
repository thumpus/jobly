...
router.get("/:handle", async function (req, res, next) {
  try {
    ...
    const jobs = await db.query(`SELECT id, title, salary, equity, company_handle
                                  FROM jobs WHERE company_handle = $1`, [company.handle])
    return res.json({ 'company': company, 'jobs': jobs.rows });
  } catch (err) {
    return next(err);
  }
});
